...
주기적으로 코드를 잘라내기/붙여넣기 하는 것을 잊지 마십시오??!!(Don't forget to cut'n paste the code on a regular basis.)
...
- X 는 오른쪽
- Y 는 위쪽
- Z 는 당신의 등쪽(뒤쪽이에요. 앞쪽이 아니라)

이를 시각화하는 더 좋은 방법ㄷ이 있습니다. 오른손 법칙을 사용하는 것입니다.

- X는 엄지 손가락.
- Y는 검지 손가락
- Z는 중지 손가락. 엄지를 오른쪽으로 향하게 하고, 검지를 하늘로 향하게 하면, 중지가 등 뒤쪽을 향하게 될 겁니다.

Z가 이 방향을 가리키는게 이상해 보이나요? 100년이 넘은 오른손 법칙 수학은 많은 유용한 도움을 줍니다. 단점은 직관적이지 않은 Z뿐입니다.

보조 메모에서 손을 자유롭게 움직일 수 있다는 것에 주목하시길바랍니다. 여러분의 X, Y, Z 또한 움직입니다. 나중에 더 자세히 살펴보도록 하겠습니다.
